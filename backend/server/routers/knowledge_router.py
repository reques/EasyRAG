...
from backend.services.ingestion_queue import publish_ingestion
...
async def upload_to_kb(
    ...
    # 发布索引任务到 Redis Stream（消息不含文件字节，MinIO 已存）。
    # 由独立 ingestion worker 进程消费（重启不丢、可横向扩展、并发可控）。
    published = await publish_ingestion(
        file_id,
        kb_uuid,
        filename,
        strategy or None,
    ...
    if not published:
        # 队列不可用 → 回退进程内后台任务（保底不丢任务，行为与旧版一致）
        logger.warning("[knowledge/upload] stream publish failed, fallback to background task")
...
async def delete_file(
    ...
    async with get_session() as session:
        ...
        # 3. 删除该文件的图谱实体/关系（实体身份 = kb + source_file + name 命名空间隔离，
        #    2026-08-27 修复：之前删文件漏清图谱，导致知识库文件删光后图中仍有残留）
        try:
            ...
            # 进程内图谱缓存同步清理（同名文件多次上传时按文件名全清，与向量删除语义一致）
            graph_cache.delete_by_source(str(kb.id), filename)
            logger.info(
                "[knowledge/delete] removed graph data for '%s' (kb=%s)",
                filename, kb.id,
            )
        except Exception as exc:
            logger.warning("[knowledge/delete] graph cleanup failed: %s", exc)
