...
def test_discover_not_gated_by_skill_context():
    """discover 的候选池不含 Skill 门控（2026-09-04 回归修复）。

    渐进式披露只约束 invoke（调用时），不能影响构建期视图 —— discover 的
    消费方（SubAgent 动态绑定）与 Agent 构建缓存同源，门控会把未激活工具
    从缓存里永久剔除。"""
    reg = _registry()
    ...
    assert {t.name for t in out} == {"web_search", "calculator"}


def test_discover_honours_skill_gate():
    """ discover 只按任务描述匹配元数据；激活状态不影响结果。"""
    reg = _registry()
    definition = SkillDefinition(
        slug="math-only", name="仅计算", description="d",
        tool_dependencies=("calculator",),
